    ...
    <main className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {Object.entries(gegroepeerdeItems)
          .sort(([keyA], [keyB]) => {
            // Zet RAL 9005 (gitzwart) altijd bovenaan
            if (keyA.startsWith('9005')) return -1;
            if (keyB.startsWith('9005')) return 1;
            return 0;
          })
          .map(([key, items]) => {
              ...
              <div className="p-5">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2 items-center">
                    {isGitzwart && (
                      <span className="bg-yellow-100 text-yellow-900 text-xs font-bold px-2.5 py-1 rounded-full border border-yellow-300">
                        Meest gebruikt
                      </span>
                    )}
                    {hasLowStock && (
                    ...
                    {isGitzwart && hasLowStock && (
                      <span className="bg-orange-100 text-orange-800 text-xs font-bold px-2.5 py-1 rounded-full border border-orange-300 ml-1">
                        Let op: veel gebruikt!
                      </span>
                    )}
                    {/* Verwijder-knop alleen zichtbaar in testfase */}
                    <button
                      className="ml-2 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 rounded-full px-2 py-1 text-xs font-bold transition"
                      title="Verwijder deze doos"
                      onClick={() => {
                        if (confirm('Weet je zeker dat je deze doos wilt verwijderen?')) {
                          items.forEach(item => {
                            verwijderVoorraadItem(item.id);
                          });
                          setVoorraadItems(voorraad());
                        }
                      }}
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                  <div>
                    ...
                    {(() => {
  // Bepaal het totaalgewicht van deze groep
  const totaalKg = items.reduce((sum, item) => sum + (item.aantal_dozen * 20) + (item.aantal_gebruikt > 0 ? (1 - item.aantal_gebruikt) * 20 : 0), 0);
  if (totaleDoosBedrag > 1) {
    return <div className="text-2xl font-bold">{totaleDoosBedrag} dozen</div>;
  } else if (totaleDoosBedrag === 1 && totaalKg === 20) {
    return <div className="text-2xl font-bold">1 doos</div>;
  } else if (totaalKg > 0) {
    return <div className="text-2xl font-bold">{totaalKg.toFixed(2).replace('.00','')} kg</div>;
  } else {
    return <div className="text-2xl font-bold">0 dozen</div>;
  }
})()}
                  </div>
                    ...
                    <span 
                      className={`inline-flex items-center gap-1 px-3 py-2 rounded-md text-sm font-medium ${
                        items[0].merk === "Tiger" 
                          ...
                          : items[0].merk === "Protech" 
                            ? "bg-red-50 text-red-700" 
                            : "bg-blue-50 text-blue-700"
                      }`}
                    >
                      <Image 
                        src={`/images/${items[0].merk.toLowerCase()}-logo.svg`}
                        alt={`${items[0].merk} logo`}
                        width={32}
                        height={32}
                        className="mr-2"
                      />
                ...
                {items.length === 1 ? (
                   ...
                   <div className="w-full mt-4">
                     <div className="mb-2 text-xs text-gray-500">Kies eerst de doos waarvoor je verbruik wil registreren:</div>
                     <div className="flex flex-col gap-2">
                       {items.map((doos, idx) => (
                         <button
                           key={doos.id}
                           onClick={() => openVerbruiksModal(doos)}
                           className="w-full bg-orange-50 hover:bg-orange-100 border border-orange-200 text-orange-700 rounded-lg py-2 px-3 flex items-center justify-between"
                         >
                           <span>Doos #{idx+1} • {doos.gewicht.toFixed(2)} kg</span>
                           <span className="text-xs text-gray-400">ID: {doos.id}</span>
                         </button>
                       ))}
                     </div>
                   </div>
            ...
            <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6 rounded-r-lg">
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-600">Huidige voorraad: {geselecteerdItem.gewicht.toFixed(2)} kg</p>
